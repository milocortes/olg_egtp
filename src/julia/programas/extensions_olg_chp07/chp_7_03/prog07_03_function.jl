...
save_path = joinpath(root, "csvs")


#function OLG((; TT_p, JJ_p, JR_p, gamma_p, beta_p, alpha_p, delta_p, xi_p, upsi_p, epsi_p, mu_p, damp_p, itermax_p, gy_p, smopec_p, policy_p))
function OLG(id_experiment, TT_p, JJ_p, JR_p, gamma_p, beta_p, alpha_p, delta_p, xi_p, upsi_p, epsi_p, mu_p, damp_p, itermax_p, gy_p, policy_p)
    try
        # model parameters
        global TT      = TT_p
        global JJ      = JJ_p
        ...
        global tol     = 1e-6
        global damp    = damp_p
        global itermax = itermax_p
        global gy = gy_p
        global smopec = false
        
        global policy = policy_p

        # model variables

        for param = [:w, :r, :wn, :Rn, :p, :tauw, :taur, :tauc, :taup, :by_p, :kappa, :n_p, :tauk, :KK, :LL, :YY, :AA, :CC, :II, :BB, :GG, :BA, :BF, :TB, 
                    :Tpen, :TXR, :taus, :e, :HH, :n_e, :tax, :eps_p]
        ...
        function factor_prices(it)
            ...
            if(smopec && it > 0)
                ...
                r[it] = (1.0-tauk[it])/(1.0-eps_p[it]*tauk[it])*(alpha*(KK[it]/LL[it])^(alpha-1.0)*HH[it]^epsi-delta)
            end
        ...
        function get_W(ij, it)
            ...
            get_W_r = winc[it, ij] + pen[it, ij]

            if(it == 1 && ij > 1)
                get_W_r = get_W_r + Rn[it]*a[it, ij]  
            end
            ...
            for ijp in ij+1:JJ
                itp = year(it, ij, ijp)
                PRn = PRn*Rn[itp]
                get_W_r = get_W_r + (winc[itp, ijp]  + pen[itp, ijp])/PRn
            end
        ...
        function get_Psi(ij, it)
            ...
            for ijp in ij+1:JJ
                ...
                get_Psi_r = get_Psi_r + beta^((ijp-ij)*gamma)*(p[itp]/PRn/p[it])^(1.0-gamma)

            end
            get_Psi_r = 1.0/p[it]/get_Psi_r
        ...
        function quantities(it)
            ...
            BB[it] = by_p[itm]*YY[it]

            # capital stock (in closed or open economy)
            if(smopec && it > 0)
                KK[it] = LL[it]*((r[it]*(1.0-eps_p[it]*tauk[it])/(1.0-tauk[it])+ delta)/alpha)^(1.0/(alpha-1.0))
        ...
        function government(it)
            ...
            taxrev[4] = tauk[it]*(YY[it] - w[it]*LL[it] - (delta+eps_p[it]*r[it])*KK[it])
        ...
        results_all[:, :id_exp] .= id_experiment

        file_save_path = joinpath( save_path,string("model_07_03_", string(id_experiment),".csv"))

        #return results_all
        CSV.write(file_save_path, results_all)
    catch e
        println("ERROR")
    end
end 

#OLG((TT_p = 24, JJ_p = 3, JR_p = 3, gamma_p = 0.5, beta_p = 0.9, alpha_p = 0.3, delta_p = 0.0, xi_p = 2.0, upsi_p = 0.5, epsi_p = 0.0, mu_p = 0.8, damp_p = 0.3, itermax_p = 200, gy_p = 0.0, smopec_p = false, policy_p = 1))

OLG( 1, 24,  3,  3,  0.5,  0.9,  0.3,  0.0,  2.0,  0.5,  0.0,  0.8,  0.3,  200,  0.0, 1)

import YAML
using OrderedCollections

parameters = YAML.load_file("parametros_07_03.yaml"; dicttype=OrderedDict{String,Any})
...
parameter_LHC = DataFrame(scaleLHC(plan, [(i[1],i[2]) for i in collect(values(parameters["parametros"]))]), Symbol.(keys(parameters["parametros"])))

policies = DataFrame((policy=[1, 2, 3])) 

experimental_design = crossjoin(parameter_LHC, policies)

experimental_design.TT = Int64.(experimental_design.TT)
experimental_design.JR = Int64.(experimental_design.JR)
experimental_design.JJ = Int64.(experimental_design.JJ)
experimental_design.itermax = Int64.(experimental_design.itermax)


experimental_design_tup = Tuple.(eachrow(experimental_design))

using ThreadPools
n_rows, n_cols = size(experimental_design)

#=
@qbthreads for (id_exp, experimento) in enumerate(experimental_design_tup)
    println("id_exper : $id_exp/$n_rows  Thread id : $(Threads.threadid())")
    OLG(id_exp,experimento...)
end
=#

for (id_exp, experimento) in enumerate(experimental_design_tup)
    println("id_exper : $id_exp/$n_rows")
    OLG(id_exp,experimento...)
end

experimental_design[:,"id_exp"] = 1:size(experimental_design)[1]
CSV.write("experimental_design.csv", experimental_design)