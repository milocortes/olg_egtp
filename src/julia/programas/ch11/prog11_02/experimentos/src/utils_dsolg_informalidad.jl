...
#
# Calculates value of function that should be integrated for pi_models.
###########################################################################
...
function simulate_AR(pi_model, shocks)
    ...
    #n = assert_eq(size(pi_model,1), size(pi_model,2), 'tauchen')
    n = size(pi_model,1)
    T = size(shocks,1)
        ...
        for ip in 1:NP
            for is in 1:NS
                ...
                for is_p in 1:NS
                    ...
                    RHS[ij, ia, ip, is, it, ik] = RHS[ij, ia, ip, is, it, ik] + pi_model[is, is_p]*margu(chelp, lhelp, it)
                    EV[ij, ia, ip, is, it, ik]  = EV[ij, ia, ip, is, it, ik] + pi_model[is, is_p]*VV[ij, ia, ip, is_p, it, ik]
                end
            ...
            for ip in 1:NP
                for is in 1:NS
                    ...
                    for ik in 1:SS
                        ...
                        for is_p in 1:NS
                            phi[ij, ial, ip, is_p, it, ik] = phi[ij, ial, ip, is_p, it, ik] + pi_model[is, is_p]*varphi*phi[ij-1, ia, ip, is, itm, ik]
                            phi[ij, iar, ip, is_p, it, ik] = phi[ij, iar, ip, is_p, it, ik] + pi_model[is,is_p]*(1.0-varphi)*phi[ij-1,ia,ip,is,itm, ik]
                        end
                    end
                end
            end
...
function aggregation(it)
    ...
    # dampi_modelng and other quantities
    ...
    HH[it]  = HH[it]/workpop    # dampi_modelng and other quantities
...
function build_parameters(country, data_file_name)
    ...
    ### Calcula alpha --> capi_modeltal share in production
...
function DSOLG(calib_params, stress_params)
    try
        ...
        println("calculate initial equilibrium")
        ...
        for param = [:pen, :c_coh, :y_coh, :l_coh, :a_coh, :v_coh, :VV_coh]
            for (ind_skill, name_skill) in enumerate(["high","low"])
                @eval col_names = [string(@Name($param),"_cohort_",i,"_",$name_skill) for i in 1:size($param.parent)[1]]
                push!(results_2D_df,
                    @eval DataFrame(OffsetArrays.no_offset_view($param[:,:,$ind_skill])', col_names)
                )
            end
        end
