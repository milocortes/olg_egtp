...
function rouwenhorst(N::Int64, ρ::Real, σ::Real, μ::Real=0.0)
    σ_y = σ / sqrt(1-ρ^2)
    p  = (1+ρ)/2
    ψ = sqrt(N-1) * σ_y
    m = μ / (1 - ρ)
...
function _rouwenhorst(p::Real, q::Real, m::Real, Δ::Real, n::Int64)
    if n == 2
...
function grid_Cons_Grow(a, n, left, right, growth)
    ...
    if(left >= right)
        error("grid_Cons_Grow : left interval point greater than right point")
    end
    # check for growth
    if(growth <= 0.0)
    ...
    # calculate factor
    h = (right-left)/((1.0+growth)^n_new-1.0)

    # calculate grid value
    
    for j in 0:n_new
        a[j] = h*((1.0+growth)^j -1.0)+left
    end
...
###############################################################################
function grid_Inv_Grow_1(x::Float64, left::Float64, right::Float64, growth::Float64, n::Int64)

    ###### ROUTINE CODE #######################################################

    # check for left <= right
    if(left >= right)
        error("grid_Inv_Grow left interval point greater than right point")
    end
    # check for growth
    if(growth <= 0.0)
        error("grid_Inv_Grow growth rate must be greater than zero")
    end
    # calculate factor
    h = (right-left)/((1+growth)^n-1.0)
    
    # calculate grid value
    return log((x-left)/h+1.0)/log(1.0+growth)
end
...
###############################################################################
function linint_Grow(x::Float64, left::Float64, right::Float64, growth::Float64, n::Int64)

    # check for left <= right
    if(left >= right)
        error("linint_Grow left interval point greater than right point")
    end
    ...
    xl = h*((1+growth)^float(il)-1.0)+left
    xr = h*((1+growth)^float(ir)-1.0)+left

    # get share on the left point
    phi = (xr-x)/(xr-xl)

    return floor(Int8,il), floor(Int8,ir), phi

end


# calculates year at which age ij agent is ij_p
function year(it::Int64, ij::Int64, ijp::Int64)
...
function year2(it::Int64, addit::Int64)
...
function margu(cons::Float64, lab::Float64, it::Int64)
    return nu*(cons^nu*(1.0-lab)^(1.0-nu))^egam/(p[it]*cons)
end 

# calculates the value function
function valuefunc(a_plus::Float64, cons::Float64, lab::Float64, ij::Int64, ip::Int64, is::Int64, it::Int64)
...
function solve_household(ij_in::Int64, it_in::Int64)
...
function interpolate(ij::Int64, it::Int64)
    for ia in 0:NA
        for ip in 1:NP
            for is in 1:NS
...
function get_distribution(it::Int64)
        ...
        for ia in 0:NA
            for ip in 1:NP
                for is in 1:NS
                    ...
                    ial, iar, varphi = linint_Grow(aplus[ij-1, ia, ip, is, itm], a_l, a_u, a_grow, NA)
